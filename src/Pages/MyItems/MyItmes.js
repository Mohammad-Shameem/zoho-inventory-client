...
const MyItmes = () => {
  ...
  const loadMyItems = async () => {
    try {
      const { data } = await axiosSecret.get(
        `https://zoho-inventory-server.up.railway.app/myitem?email=${user.email}&page=${page}&pageSize=${pageSize}`
      );
      setMyItems(data);
    } catch (error) {
  ...
  useEffect(() => {
    fetch("https://zoho-inventory-server.up.railway.app/itemcount")
      .then((res) => res.json())
      .then((data) => {
