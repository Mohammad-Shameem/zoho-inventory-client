...
const UpdateItem = () => {
    ...
    const updateQuantity = (_id) => {
        ...
        if (previousQuantity >= 1) {
            fetch(`https://arcane-caverns-12434.herokuapp.com/deliveredItem/${_id}`, {
                method: "PUT",
                headers: {
            ...
            <div className='updateItem-container'>
                ...
                <div>
                    ...
                    <div className='d-flex button-div'>
                        <button className='button ms-5' onClick={() => updateQuantity(_id)} >Delivered</button>
                        <Link to={"/inventory"}><button className="button ms-5">Manage Inventories</button></Link>
                    </div>
                </div>

            </div>
            <form onSubmit={restockItems} >
                <input className='restock-items-field' type="number" name="number" id="" placeholder='restock items' required /> <br />
                <input type="submit" value="Restock Items" className='button mt-5 mb-5' />
            </form>
        </>
    );
};
